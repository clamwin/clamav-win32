...
{
    CMbcsBuffer mbcsContainer;
    if (!mbcsContainer.FromUnicode(szContainer))
        return ERROR_NOT_ENOUGH_MEMORY;

    CMbcsBuffer mbcsProvider;
    if (!mbcsProvider.FromUnicode(szProvider))
        return ERROR_NOT_ENOUGH_MEMORY;

    return ::CryptAcquireContextA(phProv, mbcsContainer, mbcsProvider, dwProvType, dwFlags);
}
