...
int getdest(const char *fullpath, char **newname)
{
    ...
    {
        logg(LOGG_ERROR, "Internal Error please report this path %s\n", actarget);
        free(fp);
        notmoved++;
        return 0;
    }
    ...
    {
        logg(LOGG_INFO, "%s not moved/copied since already in quarantine\n", PATH_PLAIN(fullpath));
        free(*newname);
        notmoved++;
        return 0;
    }
    ...
    {
        logg(LOGG_ERROR, "Unable to find a suitable filename to move/copy %s\n", PATH_PLAIN(fullpath));
        free(*newname);
        notmoved++;
        return 0;
    }
    ...
    if ((txt = malloc(strlen(nuname) + 5)))
    {
        ...
        if ((f = fopen(txt, "w")))
        {
            if (fprintf(f, "%s\t%s", filename, nuname) != (strlen(filename) + 1 + strlen(nuname)))
                logg(LOGG_ERROR, "Unable to write quaratine information file %s\n", strerror(errno));
            fclose(f);
        }
        else
            logg(LOGG_ERROR, "Unable to write quaratine information file %s\n", strerror(errno));
        ...
        logg(LOGG_ERROR, "Unable to allocate memory for quaratine information file\n");
...
static void action_copy(const char *filename)
{
    ...
    {
        logg(LOGG_ERROR, "Cannot copy file %s (%lu)\n", PATH_PLAIN(filename), GetLastError());
        notmoved++;
    }
    free(nuname);
}
