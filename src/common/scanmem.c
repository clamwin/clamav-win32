 ...
 *        source opens.
 *
 * CreateFile is used rather than path-only APIs so the resolved name refers to
 * the already-opened object. GetFinalPathNameByHandleW (used inside
 * cli_get_filepath_from_handle) is provided by the legacy layer on Windows XP.
 *
